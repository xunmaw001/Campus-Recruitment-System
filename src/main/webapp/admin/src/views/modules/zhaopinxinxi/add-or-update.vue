<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="企业地点" prop="qiyedidian">
          <el-input v-model="ruleForm.qiyedidian" 
              placeholder="企业地点" clearable  :readonly="ro.qiyedidian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="企业地点" prop="qiyedidian">
              <el-input v-model="ruleForm.qiyedidian" 
                placeholder="企业地点" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="招聘岗位" prop="zhaopingangwei">
          <el-select v-model="ruleForm.zhaopingangwei" placeholder="请选择招聘岗位">
            <el-option
                v-for="(item,index) in zhaopingangweiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.qiyefengmian" label="企业封面" prop="qiyefengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.qiyefengmian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.qiyemingcheng = true;
          ...
          if(o=='qiyedidian'){
            this.ruleForm.qiyedidian = obj[o];
	    this.ro.qiyedidian = true;
            continue;
          }
          if(o=='zhaopingangwei'){
      ...
      this.$http({
		...
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/gangweifenlei/leixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zhaopingangweiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.qiyejieshao = this.ruleForm.qiyejieshao.replace(reg,'../../../ssmb9475/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
